...
contract Sender is OwnerIsCreator {
    ...
    function sendMessage(
        ...
        bytes32 value = bytes32(uint256(uint160(targetContract)));
        ...
        string memory addr_as_string = string(str);

        string memory concatenated = string(abi.encodePacked(addr_as_string, ",", text));

        // Create an EVM2AnyMessage struct in memory with necessary information for sending a cross-chain message
        Client.EVM2AnyMessage memory evm2AnyMessage = Client.EVM2AnyMessage({
            receiver: abi.encode(receiver), // ABI-encoded receiver address
            data: abi.encode(concatenated), // ABI-encoded string
